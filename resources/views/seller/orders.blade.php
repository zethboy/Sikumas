        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                ...
                <tbody>
                    ...
                    <tr class="border-t border-gray-100 hover:bg-gray-50">
                        ...
                        <td class="p-4">
                            ...
                            <div class="flex items-center gap-2 mb-1">
                                <img src="{{ asset($item->product->image_url) }}" class="w-8 h-8 rounded object-cover">
                        ...
                        <td class="p-4">
                            ...
                            <a href="{{ asset($order->payment_proof) }}" target="_blank" class="text-blue-600 hover:underline text-xs">Lihat Bukti →</a>
                            @else
                        ...
                        <td class="p-4">
                            <div class="flex flex-col gap-2">
                                <!-- Update Status -->
                                <form method="POST" action="{{ route('seller.orders.status', $order) }}" class="flex items-center gap-2">
                                    @csrf
                                    @method('PUT')
                                ...
                                <form method="POST" action="{{ route('seller.orders.tracking', $order) }}" class="flex items-center gap-2">
                                    @csrf
                                    @method('PUT')
                                    <input type="text" name="tracking_number" value="{{ $order->tracking_number }}" placeholder="No. Resi" class="border rounded text-xs px-2 py-1 w-24">
                                    <button type="submit" class="bg-blue-500 text-white text-xs px-2 py-1 rounded hover:bg-blue-600">🚚</button>
                                </form>
                                @endif
                            </div>
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="p-8 text-center text-gray-500">Belum ada pesanan masuk untuk produk Anda.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
